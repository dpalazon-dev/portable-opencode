...
- Reproduce a reported defect through the closest feasible user-facing path before fixing it.
- Change generated artefacts through their source or generator, never by hand.
...
- every managed resource declares source, target, owner, mode, mutability, backup, drift and verification;
- `rendered` and `copied` are defaults; `linked` requires explicit Windows evidence;
- unknown or user-owned resources are preserved, never cleaned up by absence from desired state;
...
Do not turn unrelated findings into silent scope expansion. Record them or address them in a separate coherent change unless they block correctness or verification of the current outcome.

## 12. Generated artefacts and defect fixes

- Do not manually modify files marked generated or autogenerated.
- Change the canonical source, generator or inputs and regenerate.
- Before fixing a defect, reproduce it as an end user would experience it when feasible.
- When exact reproduction is impossible, record evidence, assumptions and the substitute verification before changing code.
- Preserve original errors and full failure output before applying a fix.
...
- Use branches for spikes, executable implementation, migrations and risky changes.
...
The MVP does not require contributor governance, mandatory PRs, a public extension API, a marketplace, universal generated-project style, cross-platform parity, release trains, general dotfiles or desktop personalization, a terminal multiplexer, or support guarantees for hypothetical users.
